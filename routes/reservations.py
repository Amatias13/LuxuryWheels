...
from models import Reservation, Vehicle, PaymentMethod, ReservationExtra, Testimonial, Payment
...
def reservation():
    ...
    return render_template('reservation.html', title='Reserva',
                           vehicle=vehicle,
...
def my_reservations():
    ...
    return render_template('my_reservations.html', title='As minhas reservas', reservations=data)
...
def reserve():
    ...
    try:
        ...
        flash(f'Reserva confirmada! Total: {total_price:.2f}€')
        return redirect(url_for('reservations.my_reservations'))

    except (ValueError, TypeError) as e:
        db.session.rollback()
        flash(str(e) or 'Erro ao criar reserva', 'error')
        return redirect(url_for('reservations.reservation') + f"?vehicle_id={request.form.get('vehicle_id')}")


@bp.route('/reservation/<int:rid>/cancel', methods=['POST'])
def cancel_reservation(rid):
    if 'user_id' not in session:
        flash('Faca login para cancelar uma reserva', 'error')
        return redirect(url_for('users.login'))
    try:
...
def edit_reservation(rid):
    if 'user_id' not in session:
        flash('Faca login para editar uma reserva', 'error')
        return redirect(url_for('users.login'))
    ...
    if request.method == 'POST':
        try:
            ...
            db.session.commit()
            flash(f'Reserva atualizada! Novo total: {updated.totalPrice:.2f}€')
            return redirect(url_for('reservations.my_reservations'))
        except ValueError as e:
            flash(str(e), 'error')
